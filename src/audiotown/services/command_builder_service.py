...
from audiotown.consts.video.policy_decision import StreamDecision, VideoStreamDecision
from audiotown.consts.video.quality_profile import QualityProfile
from audiotown.consts.video.speed_profile import SpeedProfile
...
class CommandBuilderService:
    ...
    def build(
        ...
        # 3. Processing Logic -- Video and Audio Streams First
        if decision.action == MediaAction.SKIP:
            ...
            v_decision = decision.video_stream_decisions[0] if len(decision.video_stream_decisions)==1 else None
            if v_decision is not None and v_decision.tag is not None:
                argv.extend([f"-tag:v", v_decision.tag])
            logger.regular_log(f"...In CommandBuilderService... decision.action:{decision.action}...\n...first_video.codec_name: {first_video.codec_name}...\nfirst_video.codec_name==VideoCodec.HEVC.ffprobe_name: {first_video.codec_name==VideoCodec.HEVC.ffprobe_name}..")
        ...
        # MediaAction.Transcode
            ...
            for v_out_idx, v_decision in enumerate(decision.video_stream_decisions):
                ...
                if v_decision.mode == StreamDecision.COPY:
                    argv.extend([f"-c:v:{v_out_idx}", StreamDecision.COPY.value])
                    ...
                    if v_decision.encoder == VideoEncoder.LIBX264:
                        ...
                        # ])
                        argv.extend(self._build_libx264_quality_args(decision=decision,v_decision=v_decision))
                    elif v_decision.encoder == VideoEncoder.LIBX265:
                        argv.extend(self._build_libx265_quality_args(decision=decision,v_decision=v_decision))
                        # argv.extend([
                        #     f"-crf:v:{v_out_idx}", decision.speed_profile.value if decision.speed_profile is not None else str(20),
                        #     f"-preset:v:{v_out_idx}", "medium",
                        #     f"-pix_fmt:v:{v_out_idx}", v_decision.pixel_format if v_decision.pixel_format is not None else PixelFormat.YUV420P10LE,   # or yuv420p10le"
                        # ])
                    # if v_decision has other than libx265 or libx264
                    else:
                        raise TypeError(f"Unsupported video codec is specified: {v_decision.codec}")
            ...
            for audio_out_idx, au_decision in enumerate(decision.audio_stream_decisions):
                ...
                if au_decision.mode == StreamDecision.COPY:
                    argv.extend([f"-c:a:{audio_out_idx}", StreamDecision.COPY.value])
        ...
        # 4. Process Subtitle Mapping: Only map what we can actually handle
        if video_record.has_subtitle:
            ...
            if (
                len(eligible_subs) > 0
                and decision.subtitle_mode == SubtitleMode.MOV_TEXT
            ):
                ...
                for idx, stream in enumerate(eligible_subs):
                    ...
                    argv.extend([f"-c:s:{idx}", SubtitleMode.MOV_TEXT.value])

        # if decision.prefer_english_audio_default and decision.prefer_english_subtitle_default:
        argv.extend(
            self._apply_language_and_default_preferences(
                video_record=video_record, decision=decision
            )
        )
    ...
    def _build_libx264_quality_args(self, decision: PolicyDecision, v_decision: VideoStreamDecision) -> list[str]:
        args = []

        # 1. Handle Preset
        preset = decision.speed_profile.value if decision.speed_profile else SpeedProfile.MEDIUM.value
        ...
        # if v_decision.pixel_format == PixelFormat.YUV420P:
        args.extend(["-pix_fmt", f"{v_decision.pixel_format.value if v_decision.pixel_format is not None else PixelFormat.YUV420P.value}"])
    ...
    def _build_libx265_quality_args(self, decision: PolicyDecision,v_decision: VideoStreamDecision) -> list[str]:
        args = []
        ...
        # 3. Handle Pixel Format
        # if  v_decision == PixelFormat.YUV420P:
        args.extend(["-pix_fmt", f"{v_decision.pixel_format.value if v_decision.pixel_format is not None else PixelFormat.YUV420P10LE}"])
    ...
    def _generate_output_path(
        ...
        return str(output_folder / video_record.file.with_suffix(target_container.suffix).name)
    ...
    def _build_default_disposition_args(
        ...
        # That clears disposition flags for every output audio/subtitle stream of that type.
    ...
    def _apply_language_and_default_preferences(
        ...
        output_audio_streams = video_record.audio_streams
        output_subtitle_streams = [sub for sub in video_record.subtitle_streams if sub.is_mp4_text_compatible]
