...
ARRAY_RANGES = {
    8: (-0x80, 0x7f),  # (-128, 127)
    16: (-0x8000, 0x7fff), # (-32_768, 32_767),
    32: (-0x80000000, 0x7fffffff), # (-2_147_483_648, 2_147_483_647),
}
...
def get_array_type(bit_depth, signed=True) -> str:
...
def _fd_or_path_or_tempfile(fd, mode="w+b", is_temp_file: bool=True):
    close_fd = False
    if fd is None and is_temp_file:
...
def get_prober_name() -> str | None:
    """
    Return probe application, either avconv or ffmpeg
    """
    path_str = shutil.which("ffprobe")
    if path_str:
        return path_str
